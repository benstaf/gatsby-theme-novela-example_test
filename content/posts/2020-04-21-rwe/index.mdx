...
However, most clinical results either involve small cohorts, and/or lack a control group.

These methodological weaknesses generate heated controversies on social medias.

![](https://paper-attachments.dropbox.com/s_3AEED641DCCE94BAD97655461814D8A7BCFCFF1E38325BF0C342CE79714C0B94_1587484705883_014-Thumbnail+1+1.png)

# Randomized Controlled Trials vs. Real World Evidence

  

These controversies reflect the old methodological debate between Randomized Controlled Trials  (RCT) and Real World Evidence  (RWE)  (for a quick introduction, see [here](https://www.forbes.com/sites/davidshaywitz/2018/05/11/will-real-world-performance-replace-rcts-as-healthcares-most-important-standard/#41eb890f3557), or listen to this [podcast by a16z](https://16minutes.simplecast.com/episodes/16mins-rwe-rct-real-world-evidence-data-randomized-controlled-trial-fda-drugs-coronavirus-pandemic) from 10:21).
...
The reason is quite intuitive: to simplify, consider 2 datasets with 2 patients each, and 2 comorbidities: age and obesity. These 2 datasets can be equivalent at the aggregated level, but very different at the individual level. They give different clinical outcomes:
...
Propensity scores have to be calculated from available data. That's where AI and machine learning come into play. The basic algorithm is logistic regression: the output of this classifier is a probability, between 0 and 1, giving the propensity score of each patient.
...
Finally, there are several methods to make the [matching itself](https://www.statisticshowto.com/propensity-score-matching/). For example, one way is to match each treatment patient with its nearest neighbor in the control dataset, except when they are too far from each other  (distance above a tolerance threshold).
